...
[some *corresponding tab* text]{style="color:blue"}. This feature
...
filters out natural barriers and any barriers situated upstream of
natural barriers.
:::
...
::: columns
::: {.column width="65%"}
Refining culverts based on Area, Subarea, and Ownership sets the benefit
of any culvert outside the selection to 0. Although a culvert outside
the selected area, subarea or ownership could still end up in the
solution, especially if it is needed to restore the culvert outside to
get any benefits at all as it is very possible another ownership type
and area is critical. The tool will point this out.
...
populate with subareas contained within the selected Areas after
clicking the submit button at the bottom of the left panel.
...
The highlight barriers field allows specific culverts to be selected by their ID from the WDFW barrier inventory. This will highlight them specifically both on the map as well as on the scatterplot. 
...
1.  An interactive map showcasing culverts within the selected area,
    subarea, and ownership.
2.  A clickable scatter plot illustrating the chosen characteristics.
...
::: columns
::: {.column width="55%"}
The Suggest feature returns a restoration plan that maximizes habitat,
given the users budget, project area, and other constraints.
...
your project area, define your budget, and choose the culvert ownership
types. This process, accessible in the Suggest section, is
straightforward and user-friendly, allowing you to quickly set the
parameters for your habitat restoration plan.
:::
...
::: columns
::: {.column width="65%"}
As in Explore, culverts can be refined based on Area, Subarea, and
Ownership sets the benefit of any culvert outside the selection to 0.
Although a culvert outside the selected area, subarea or ownership could
still end up in the solution, especially if it is needed to restore the
culvert outside to get any benefits at all as it is very possible
another ownership type and area is critical. The tool will point this
out.
...
their Suggest analysis by inputting new information and customizations.
...
passability is not yet reflected in the app's data) or are already
...
By marking these culverts as planned or completed, you effectively
inform the planning tool to consider them as fixed assets in its
optimization algorithm. This approach allows for more strategic planning
and coordination around other known projects, ensuring that resources
are allocated efficiently and effectively. The culverts marked as
'Already Planned / Will Complete' will be excluded from the cost
calculations in the analysis, but their associated habitat will still be
displayed on the map for informational purposes. However, these culverts
will be ignored in the generation of new reports to focus on the
remaining barriers that require attention
:::
...
Complete field.Once added, click the submit button again.
...
not provide habitat for the selected species, or if it's deemed
unsuitable for passage, the benefit of modifying or repairing that
culvert is set to 0 in the context of the selected species. This ensures
...
The Objective feature in Upstream provides a more comprehensive analysis
of habitat quality. When selecting 'Weighted Attributes', the system
...
natural areas. This weighting system enables users to make localized
assessments of habitat quality based on land use. Note: Weights must sum
to 1.
...
likely to encounter. The temperature values are pulled from the
...
Missing temperature values were interpolated using IDW with n.max = 10
...
disposal. Cost estimates were initially established using methods
described in [**Van Deynze et al
2022**](https://www.sciencedirect.com/science/article/abs/pii/S0006320722002634).
As these were 2019 costs, CPI is considered and used to adjust the
estimates to the current year for inflation. Unknown costs are spatially
interpolated using IDW.

<img src="images/suggest_cost.jpg" style="width: 50%; display: block; margin-left: auto; margin-right: auto;"/>

-   **Provide Mean Project Cost:** Users can enter the average cost of either the design phase or the construction phase of their culvert projects. This input is invaluable for those who have detailed, project-specific financial data. The mean project cost provides a baseline for the design cost of the culvert. Additionally, construction costs are based on the regional averages as cited in the literature, which allows for the scaling of costs to adjust to the relevant project size and scope. This scaling is achieved by applying a ratio of the construction cost to the user's specific project costs, ensuring that estimates reflect the unique aspects of each project.

-   **Default Modelled Cost Estimates:** For users without specific cost data, Upstream offers a default option that utilizes modelled cost estimates. These estimates are based on a comprehensive analysis of regional data and provide a reliable approximation for project budgeting. The modelled estimates are continually updated to reflect current economic conditions and provide a robust starting point for users who do not have access to specific cost data.


This dual approach to cost estimation ensures that users can tailor the financial aspects of their habitat restoration plans according to the level of detail they have, ensuring both accuracy and flexibility in project planning. By providing a mean project cost and allowing for the addition of average design costs, Upstream facilitates a comprehensive and adaptable framework for financial planning. The system's ability to scale costs according to specific project parameters ensures that users can make informed decisions that reflect both the scope of their projects and the nuances of regional cost factors.
...
::: columns
::: {.column width="55%"}
Once any of the prior fields are populated, provide the budget available for repairs in US dollars---such as 80000 in the example to the right.

For example, on the right we allow for Upstream to suggest
culverts of all ownership types in all sub-areas within the
Chambers-Clover WRIA only. All other fields were left unpopulated.
...
1.  A map highlighting, in blue, barrier(s) that are estimated to unlock
    the most habitat for the user defined budget. selections here are
    based on [Recent
    research](https://www.sciencedirect.com/science/article/abs/pii/S0006320722002634)
    on regional repair costs, and uniform habitat quality.
...
2.  A report, matching projects on the map, that provides the summed
    habitat by fish species (if known), that is estimated to be
    repaired, along with estimated cost per barrier id within the plan.
...
Including the two culverts to the southwest as already planned /
completed, and leaving budget the same, suggests a new third culvert to
the northeast, the map indicates potential habitat gains in blue with
areas that will remain blocked in red.
:::
...
Only the unlocked habitat and cost of the new suggestion is given. Note
that this particular stream had no species listed, but still reports the
total linial gain.
...
1.  Introduction This section can be used to describe potential use
    cases.
