...
-   *Welcome:* Familiarize yourself with the culvert inventory.
...
WRIAs. Points with light gray boarders are those that we consider "Good
...
attributes from the WDFW inventory, which includes a link to the
culvert's site report link. If you are interested in learning more about
...
filters out corrected barriers, natural barriers, and any barrier
culverts situated upstream of natural barriers.
:::
...
Here you determine if you would like to view a scatterplot output of a histogram output depending on the selected culverts and attribute or attributes of interest. 

Once plot type has been selected, you are prompted to select variables to display. If you select a histogram, you can select 1 variable and then the number of bins from 1 to 100 to visualize the variables distribution across the selected culverts. 

![](images/explore_hist_select.jpg)

If you select scatterplot, you will be prompted to select an X and a Y variable to display. As there can be overlap, a jitter option is the provided for both variables. 
...
Additionally, we can select variables to color the culvert points. This
will color them on the generated plot, whether that is a scatterplot or
a histogram, as well as on the interactive map. For continuous
variables, colors are displayed based on the variables being grouped
into 5 quantile ranges since many variables are skewed. If you refine
your selection spatially or for ownership type, the quantile groups will
then only reflect this refined range. 
...
on the map as well as in the scatterplot.
...
The default selection is to remove bad matches, however, in this section the user can include bad matches in the data. Bad matches are those that were identified as not likely associated with an NHD line. More info can be found in the Learn section of this manual. 
...
1.  An interactive map showcasing culverts within the selected area
    (outlined in dark blue), subarea, and ownership. Note that any
    highlighted culverts are outlined in bright, light blue. Clicking on
    the culverts in the map also will trigger a popup window displaying
    additional attributes.

2.  A clickable plot illustrating the chosen characteristics.
    Again, if highlight barrier(s) is selected and barriers are added,
    then these will be circled as bright blue within a scatterplot.
...
To demonstrate this tool, we will analyze the relationship between the temperature of all
upstream habitat weighted by area and the area of all blocked habitat:
...
interactive map. From this example we see 2 groups of culverts: A
relatively low temperature above and a relatively high temperature.
...
Below is an example of if we fill
in the points by selecting "Full Temperature (Area)" within the Select
...
::: columns
::: {.column width="65%"}
If we now refine the area by selecting a specific WRIA (Kennedy - Goldsborough), we can see that
the quantile color has shifted in the scatterplot.
:::
...
Using the examples in the Kennedy - Goldsborough WRIA, we select 115 MC205 since it is the outlier for Full Habitat Area. After hitting Submit again, we see this culvert highlighted in both the scatterplot and the interactive map in lightblue. 
...
::: columns
::: {.column width="55%"}
The Suggest feature returns a restoration plan that maximizes a flexible
objective, given the users budget, project area, and other constraints.
...
your project area, the culvert ownership types of interest, define your
objective, and provide a budget. This process, accessible in the Suggest
section, is intended to be flexible and user-friendly, allowing you to
quickly set parameters for your a suggested habitat restoration plan.
:::
...
::: columns
::: {.column width="65%"}
In the Suggest feature, refining culverts based on Area, Subarea, and
Ownership sets the benefit of any culvert outside the selection to zero.
Note that a culvert outside the selected area, subarea or ownership
...
greatest benefit within the solution. The tool will point this out.
...
The 'Select Species of Interest' button allows users to tailor the analysis to
...
not provide habitat for the selected species the benefit of modifying or
repairing that culvert is set to zero in the context of the selected
species. This ensures that the analysis and subsequent recommendations
are specifically targeted to improve conditions for the species of
interest, allowing for more effective and targeted conservation efforts.
...
their Suggest analysis through providing data and customizing the
objective. The 'Select Projects to Ignore' field allows users to
input the IDs of culverts that have either been recently completed (but
whose corrected status is not yet reflected in the app's data) or are
already planned and budgeted for repair separately. When you include
these culverts, the analysis will not allocate any of your budget
towards them. Instead, it will assume they are already fixed and
passable, allowing the tool to suggest additional barriers for repair
...
but their associated habitat will still be displayed on the map for
informational purposes as tan.
:::
...
Below we can see an example of a solution within the Kennedy - Goldsborough WRIA with a project selected to ignore along with solutions in blue. 
...
The Objective feature in Upstream allows consideration of habitat
quality. When selecting 'Weighted Attributes', the system incorporates
various habitat quality metrics, building upon the chosen Habitat
Quantity Metric.
...
weight on forested habitat). Note: Weights must sum to 1.
...
-   **Default Modelled Cost Estimates:** For users without specific cost
    ...
    estimates. These estimates are based on a comprehensive analysis of
    regional data and provide a reliable approximation for project
    budgeting. The modeled estimates are continually updated to reflect
    current economic conditions and provide a robust starting point for
    users who do not have access to specific cost data.
...
1.  A map highlighting, in blue, barrier(s) that are estimated to unlock
...
2.  A report, matching suggested projects on the map, that provides the
    total estimated habitat gains, by fish species (if known), along
    with estimated cost per barrier id within the plan.
...
Now let's consider prioritizing Coho habitat adjusting the Mean Design & Permitting Cost and the Mean Construction Cost to $500,000. We will also consider a budget of 6e6 within the Kennedy - Goldsborough WRIA for all ownership types. 
...
In the new map, we see that overall habitat length gained by this solution as shown by the thick
blue lines. To see the details associated with the individual projects, we can view the Report tab.
...
If we then consider solutions that only consider agricultural and natural surrounding landscape types and those with upstream temperatures below 17 degrees Celsius, we can do so by adjusting the Objectives inputs. Note: Here we set urban land cover to 0, therefore the other two land type weights must equal 1. We evenly weigh them as 0.5 and 0.5. 
...
::: columns
::: {.column width="65%"}
As in the Explore and Suggest feature, we can refine culverts based on Area, Subarea, and
Ownership setting the benefit of any culvert outside the selection to zero.
Note that a culvert outside the selected area, subarea or ownership
could still end up in the suggested solution. This will happen if
restoring culverts outside of the selection is required to achieve the
greatest benefit within the solution. The tool will point this out.
...
As in the other features, habitat quantity can be quantified in terms of length, area, or volume. By default,
length is used as the primary measure.
...
As in the Suggest feature, users have the flexibility to input
specific financial details for their projects. This feature is
particularly useful for those who have precise cost information at their
disposal.
...
::: columns
::: {.column width="55%"}
Again, the 'Select Projects to Ignore' field allows users to
input the IDs of culverts that have either been recently completed (but
whose corrected status is not yet reflected in the app's data) or are
already planned and budgeted for repair separately. When you include
these culverts, the analysis will not allocate any of your budget
...
This is the field that allows you to enter culverts based on their IDs to view the estimated costs as well as the benefits. If you provided any constraints in the above fields, the culvert IDs will reflect them. Note: You must provide at least 1 culvert ID to generate an output within the Custom feature. 
...
as our selected area considering all HUC 12s and ownership types. We will provide the Mean Design & Permitting Cost and the Mean Construction Cost as $500,000 and the ID for the culvert will be 115 MC097, which we enter in the Enter ID(s) for Your Custom Plan field. 
...
culvert in blue showing their location and the habitat that they open
up. Since there is a downstream barrier, none of the habitat above the selected culvert appears blue as it is not opened up with this project alone. 

However, if we know that the downstream culvert is already being corrected and will not need to be funded directly by our project, we can add it to the Select Projects to Ignore field. In this example, we will add 115 MC040 to the Select Projects to Ignore. Now we see in the solution that the habitat above our culvert is now considered accessible by being marked blue with the below habitat as tan. 
...
Some culverts are well beyond a distance that allows them to be linked
to any NHD line. We developed a machine learning solution considering lidar burn-in
lines to help establish which culverts are beyond a confident match with a stream line. 
